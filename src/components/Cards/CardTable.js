...
//   // The handleEdit function now calls the parent's onEdit prop
//   const handleEdit = (itemId) => {
//     setLoading(true);
//     fetch(`${backendUrl}/api/items/${itemId}`)
//       .then((res) => res.json())
//       .then((data) => {
//         if (data.success) {
//           // Pass the item data back to the parent component
//           onEdit(data.data); 
//         } else {
//           toast.error("Failed to fetch item details.");
//         }
...
// CardTable.propTypes = {
//   color: PropTypes.oneOf(["light", "dark"]),
//   // Add prop type for the new onEdit prop
...
export default function CardTable({ color, onEdit }) {
  ...
  const handleEdit = (itemId) => {
    ...
    fetch(`${backendUrl}/api/items/${itemId}`)
      ...
      .catch((err) => {
        console.error("Error fetching item for edit:", err);
        toast.error(`Error fetching item details: ${err.message}`);
      })
      .finally(() => {
        setLoading(false);
      });
